...
def paper_trading_status():
    ...
    if account.strategy == "sweeper" and not account.is_active:
        paused_reason = (
            "익일 14:30 근사가 체결 기준(하루 한 번, 실전과 동일 시각에 확인해 주문한다고 가정)으로 "
            "재검증한 결과 손절폭을 넓혀도(6~25%) CAGR -30%~-32%, MDD -98%~-99%로 손실 위험이 커 "
            "모의투자를 중단했습니다. 과거에 보였던 +53% CAGR은 저가 즉시체결·상장폐지 강제청산 등 "
            "비현실적인 체결 가정이 만든 결과였습니다. 기존 보유 내역은 그대로 남겨두었으며 더 이상 "
            "매매는 진행되지 않습니다."
        )
