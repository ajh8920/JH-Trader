...
def paper_trading_status():
    ...
    if account.strategy == "sweeper" and not account.is_active:
        paused_reason = (
            "종가 확정 기준 체결(장중 저가/고가 터치 즉시체결 가정 제거)로 재검증한 결과 손절폭을 "
            "넓혀도(6~25%) CAGR -22%~-32%, MDD -97%~-99%로 손실 위험이 커 모의투자를 중단했습니다. "
            "과거에 보였던 +53% CAGR은 저가 즉시체결·상장폐지 강제청산 등 비현실적인 체결 가정이 만든 "
            "결과였습니다. 기존 보유 내역은 그대로 남겨두었으며 더 이상 매매는 진행되지 않습니다."
        )
