...
# "와쳐 2.1(Watcher v2.1)" - "무조건 지인의 조건(전체)에 닿을 수 있는 전략을
# 찾으라"는 요청에 51~62차(12라운드, 190개+ 변형)로 답한 결과. 재평가 간격은
# 와쳐(v1)와 동일하게 3일로 유지한다(1일로 바꾸면 39~50차에서 확인했듯
# 오히려 전부 나빠진다). 두 단계로 나눠 찾았다:
#
# 1단계(51~57차) - 승률만 겨냥: 눌림목 지속일(5일)+손절폭 확대(5%)+시간손절
# 완화(0.25R)+MA이탈 완화(4일)+분기EPS성장 20%↑ 요구 조합이 승률·CAGR을
# 동시에 개선(22.6%->28.2%, 40.06%->27.43%)했지만, 평균수익(31.56%, 목표의
# 60%)·손익비(4.48, 목표의 48%)가 더 큰 격차로 남았다.
#
# 2단계(58~62차) - 평균수익·손익비·CAGR·거래빈도까지 종합해서 다시 탐색:
#  - 챈들리어 트레일링 4배(3배->4배, 58~59차) - 급등 초입에 너무 일찍
#    추적손절로 잘려나가던 큰 승자를 살린다. 예상 밖으로 승률/CAGR도 같이
#    올랐다(3배가 오히려 좋은 거래까지 조기 청산하고 있었다는 뜻).
#  - 눌림목 지속일 5일->3일(60차) - 5일은 승률엔 최선이었지만 후보를 너무
#    줄여 거래빈도(목표 연164건)를 갉아먹었다. 3일이 평균수익 하락 없이
#    거래량을 크게 늘렸다.
#  - RS 상대순위 85->80, 손절폭 3.5%(61~62차) - 후보 유입을 더 늘리면서도
#    성과가 오히려 개선되는 지점을 찾았다.
# 실측(2016-01-01~2026-09-19, 10.7년): CAGR 36.34%(목표의 72%), MDD -27.00%
# (목표 -44.45%보다 훨씬 안전), 승률 21.9%(목표의 69%, 1단계보다는 낮아짐 -
# 아래 트레이드오프 설명 참고), 평균수익 36.97%(71%), 손익비 6.31(68%),
# 연 81.4건(50%). 종합 적합도(5개 지표 평균 달성률)로는 1단계(70%)보다
# 2단계(76%)가 더 낫지만, 승률 자체만 보면 1단계가 더 높다(28.2% vs 21.9%) -
# "승률 우선"과 "전체 지표 균형" 중 무엇을 택하든 트레이드오프가 있다는 뜻이고,
# 이 세션에서는 후자(2단계, 종합 균형)로 최종 채택했다.
# 남은 가장 큰 격차는 거래빈도다(연 81.4건, 목표의 50%) - 후보군을 더 넓히면
# (RS 문턱을 더 낮추거나 지속일을 1~2일로) 오히려 MDD가 급격히 악화된다(60차
# FJ/GD 실측: MDD -36%~-51%) - 단순히 "더 많이 사면" 해결되는 문제가 아니다.
# 공시 촉매 요구(require_catalyst)는 시도했으나(55차 AG/AH) 후보가 너무 줄어
# CAGR이 붕괴해(1~5%) 기각했다. 사용자 요청으로 기존 와쳐(v1, 재평가 3일
# 유지 - rescan_interval_days=3)는 그대로 두고 이 조합을 별도 전략
# ("와쳐 2.1")으로 분리했다.
WATCHER_V21_PARAMS = {**WATCHER_PARAMS,
    "pullback_min_persist_days": 3, "max_initial_risk_pct": 3.5,
    "time_stop_progress_r": 0.25, "ma_break_consec_days": 4,
    "min_eps_growth_pct": 20.0, "chandelier_atr_mult": 4.0,
    "evan_params": {"min_rs": 80.0},
}
...
def passes_evan_stage2(closes, highs, i, rs_rating,
    ...
    2026-08-27 실측 스냅샷(1단계 361종목, 2단계 12종목)에서 처음 역산했다.
    아래 6개 조건을 모두 만족하는 종목이 그날의 2단계 12종목과 정확히
    일치했다(오탐·누락 0).

    09-04/09-08 스냅샷(1단계 580/579종목, 2단계 20/16종목, 누적 90건)으로
    재검증한 결과 경계값 3개가 더 낮아야 실측과 더 잘 맞았다(12개월수익률
    40.0->32.0, 200일선이격도 8.0->6.0, 3개월수익률 -6.0->-9.5 - 근거 종목:
    제이에스코퍼레이션·한올바이오파마). 하지만 이 완화된 값으로 와쳐
    백테스트를 재검증하니 CAGR이 47.83%->41.71%(목표의 94%->82%)로,
    총수익이 +6419%->+4048%로 떨어졌다(승률만 22.6%->23.3%로 소폭 개선 -
    이번에도 익숙한 트레이드오프). "지인 실제 조건에 더 정확히 맞추는 것"과
    "우리 백테스트 성과"가 이번에도 반대 방향으로 움직인다는 뜻이라, 아래
    기본값은 원래(08-27 기준, 더 타이트한) 값으로 유지한다 - 참고용으로
    실측 경계값만 남겨둔다.

    6개 조건을 전부 만족해도 2단계에 못 드는 종목이 매 스냅샷 반복 관측된다
    (한화·GS리테일·코미팜·케이씨·마이크로컨텍솔 등) - 수급(외국인/기관
    순매수)·실적성장 연속성·베타·변동성·ROE추세까지 비교해도 이들만의 공통점을
    못 찾았다. 즉 이 6개 조건은 "필요조건"이지 "충분조건"이 아니고, 우리가
    특정 못한 추가 조건이 최소 하나 더 있는 것으로 보인다.

    주의: 역산에 쓴 임계값은 관측된 통과 종목들의 '최솟값'이라 실제 기준선은
    이보다 낮을 수 있다(예: 실제가 RS>=65여도 관측된 최저 종목이 70.3이면
    같은 결과가 나온다) - 스냅샷이 쌓일수록 계속 낮아질 가능성이 있다.
